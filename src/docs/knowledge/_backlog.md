...
> 持续收集用。每完成一项，在 `knowledge/index.ts` 注册并划掉；新想法随时追加。
...
- [X] **Node / Express 后端面试**（本项目后端栈，补全全栈视角）— `node-backend.md`，已发布
- [X] **数据库与 MySQL 优化**（索引 / 事务 / 慢查询，对应 VueChestServer）— `mysql-optimization.md`，已发布
