...
<!-- niuke-id:NQ-057 source-line:90 -->

**问题：** React 和 Vue 的区别？各自适合什么场景？React 和 Vue 在 Diff 算法上的核心区别？React 与 Vue 虚拟 DOM 的区别？React 与 Vue 双向绑定的区别？Vue / React 各自的优缺点？
...
<!-- niuke-id:NQ-058 source-line:91 -->

**问题：** React 常用版本？React 18 / 19 有哪些新特性（Actions / use hook / Server Components / cache() / useTransition 等）？React17 与 React18 升级点？
...
<!-- niuke-id:NQ-059 source-line:92 -->

**问题：** React 主要流程介绍（从 setState 到渲染）？React 为什么只更新部分 DOM？state 执行后发生了什么？反复调用 setState 是一次渲染还是多次渲染？
...
<!-- niuke-id:NQ-060 source-line:93 -->

**问题：** 了解虚拟 DOM 吗？setState → 虚拟 DOM → 真实 DOM 的完整过程？虚拟 DOM 的优缺点、性能真的更好吗？
...
<!-- niuke-id:NQ-061 source-line:94 -->

**问题：** React 常用 Hooks 有哪些？各自作用？useEffect 是第一参数入参和返回值是什么、执行机制与原理？useLayoutEffect 和 useEffect 的区别、与 requestAnimationFrame 的执行顺序？
...
<!-- niuke-id:NQ-062 source-line:95 -->

**问题：** useState 可以放在条件语句里吗，为什么？useState 返回为什么是数组不是对象？
...
<!-- niuke-id:NQ-063 source-line:96 -->

**问题：** 了解 React Fiber 吗？为什么需要 Fiber 架构（可中断渲染）？引入 fiber 架构的原因、可中断内部如何实现、何时中断节点渲染、浏览器如何验证空闲时间？React 如何实现中断？fiber 的数据结构与算法？
...
<!-- niuke-id:NQ-064 source-line:97 -->

**问题：** 函数组件和类组件的区别？Hooks 的优缺点？为什么要有 React Hooks、设计动机？Hooks 为什么不能放在分支逻辑（if）里？依赖过多问题怎么解决（useMemo / useCallback / 拆分 Hook）？用 useMemo 实现 useCallback？
...
<!-- niuke-id:NQ-065 source-line:98 -->

**问题：** 受控组件与非受控组件？React 父组件更新如何防止子组件不必要的更新（React.memo / shouldComponentUpdate）？React 做优化会用哪些 hooks？
...
<!-- niuke-id:NQ-066 source-line:99 -->

**问题：** React 状态管理工具选择与实践（Redux / Zustand / Pinia）？为什么选 Redux / 不用 redux 如何实现、redux 中间件？为什么选 Pinia？
...
<!-- niuke-id:NQ-067 source-line:100 -->

**问题：** 自己封装的 hook 介绍？有没有写过自定义 hooks？
...
<!-- niuke-id:NQ-068 source-line:101 -->

**问题：** React Native 为什么用 React 写 UI？react-dom 了解过吗？react native 原理？H5 和 React Native 的区别？
...
<!-- niuke-id:NQ-069 source-line:102 -->

**问题：** JSX / TSX 的编译原理（编译成 React.createElement）？
...
<!-- niuke-id:NQ-070 source-line:103 -->

**问题：** Diff 算法的核心原则？时间复杂度怎么从 O(n³) 优化到 O(n)？key 的作用？React 如何跨层级通信？
...
<!-- niuke-id:NQ-071 source-line:104 -->

**问题：** 组件库项目有什么比较复杂的组件、怎么实现的？自研组件库亮点难点（如何封装一个 button / ProTable / ProForm）？组件库如何升级？
...
<!-- niuke-id:NQ-072 source-line:105 -->

**问题：** 微前端怎么做的？微前端如何做状态保持？qiankun 接入步骤 / 子应用配置打包 / 沙箱与通信？
...
<!-- niuke-id:NQ-073 source-line:106 -->

**问题：** 在 React 里怎么接入 Vue 组件（Web Component 包装）？前端里怎么接入 AI（流式渲染、上下文传给大模型）？
...
<!-- niuke-id:NQ-074 source-line:110 -->

**问题：** Vue2 和 Vue3 的区别？Vue3 的响应式原理（Proxy）？Vue2 如何监听数组的 push / pop？Vue2 直接通过数组下标赋值无法更新视图的原因与解决方案？Vue2 响应式的弊端、哪些数组方法改动不会被监听？
...
<!-- niuke-id:NQ-075 source-line:111 -->

**问题：** Proxy 与 defineProperty 的区别？Vue3 中如何避免一开始递归加响应式带来的性能问题？为什么用 Proxy，与 Vue2 defineProperty 相比解决什么问题？
...
<!-- niuke-id:NQ-076 source-line:112 -->

**问题：** 组合式 API 与选项式 API 的区别、优势？组合式函数（composables）是什么、怎么写？组合式 API 的优缺点？hooks 与 mixin 的区别？
...
<!-- niuke-id:NQ-077 source-line:113 -->

**问题：** ref 与 reactive 的区别？为什么 template 里响应式不用 .value？
...
<!-- niuke-id:NQ-078 source-line:114 -->

**问题：** Vue3 生命周期？Vue2 与 Vue3 的生命周期对应？Vue3 的 Composition API setup() 里响应式丢失的原因与解决？第一次渲染触发哪几个钩子、created 与 mounted 的区别？
...
<!-- niuke-id:NQ-079 source-line:115 -->

**问题：** v-if 与 v-show 的区别及渲染机制？visibility:hidden、display:none、opacity:0 的核心区别？
...
<!-- niuke-id:NQ-080 source-line:116 -->

**问题：** provide / inject 的数据是否响应式？mixin、slot 的理解与使用？插槽的使用场景？
...
<!-- niuke-id:NQ-081 source-line:117 -->

**问题：** keep-alive 的作用与原理？
...
<!-- niuke-id:NQ-082 source-line:118 -->

**问题：** Vue 的响应式原理？双向数据绑定的底层实现？v-model 的实现原理（input / select）？
...
<!-- niuke-id:NQ-083 source-line:119 -->

**问题：** 组件通信方式有哪些（父子组件通信、跨层级）？全局状态管理怎么做的？
...
<!-- niuke-id:NQ-084 source-line:120 -->

**问题：** Vuex 的原理、含义和作用？为什么选 Pinia？Pinia 相比 Vuex 的优点、两者使用场景区别？Pinia 做状态管理的意义与价值？Pinia 怎么做响应式持久化、与 Vuex 在插件机制上的本质差别？vuex 迁移到 pinia 的原因？
...
<!-- niuke-id:NQ-085 source-line:121 -->

**问题：** Vue Router 介绍？两种模式（hash / history）区别？页面权限 / 路由鉴权怎么实现？动态路由、RBAC 权限整套实现流程？
...
<!-- niuke-id:NQ-086 source-line:122 -->

**问题：** Vue 列表渲染中 key 的作用及对应底层渲染逻辑？用数组下标作为 key 会带来哪些渲染异常（不止插入元素）？即使严格响应式规范操作，还有哪些情况可能渲染异常？
...
<!-- niuke-id:NQ-087 source-line:123 -->

**问题：** Vue nextTick 是什么、作用？如何结合 nextTick 优化流式渲染？vue observable 了解吗？
...
<!-- niuke-id:NQ-088 source-line:124 -->

**问题：** 为什么用 Vue3 / 为什么选 Vue？Vue 是怎么编译的？
...
<!-- niuke-id:NQ-089 source-line:125 -->
...
<!-- niuke-id:NQ-090 source-line:126 -->

**问题：** 一个 Vue2 的 H5 项目想迁移到 Vue3 该怎么做（迁移方案、破坏性变更兼容、组合式 API 改写、依赖与构建工具升级、风险控制）？
...
<!-- niuke-id:NQ-091 source-line:127 -->

**问题：** history 模式刷新时为什么会出现白屏 / 404？怎么解决（服务端配置 SPA fallback / historyApiFallback）？
...
<!-- niuke-id:NQ-092 source-line:128 -->

**问题：** 用 reactive 声明了一个表单对象，想把它的字段重置回初始值，应该怎么做（为什么不能直接 `formData = initialData` 整体重新赋值来重置、Object.assign 合并初始值为什么可以）？
...
<!-- niuke-id:NQ-093 source-line:132 -->

**问题：** 组件库如何适配暗黑模式？CSS Module 用过吗、原理？class 冲突怎么解决？修改 antd 组件样式如何避免被覆盖？styled-components 使用与实现原理？Shadow DOM 样式隔离？Tailwind 相比普通 css 的优势、类名太多如何维护？
...
<!-- niuke-id:NQ-094 source-line:133 -->

**问题：** less / sass 原理？webpack 中如何配置编译 less？
...
<!-- niuke-id:NQ-095 source-line:134 -->

**问题：** 单页面路由的理解；SPA 和 MPA 的区别及优缺点？为什么只有一个 index.html 却能显示不同页面（SPA）？
...
<!-- niuke-id:NQ-096 source-line:135 -->

**问题：** 表单有很多字段，如何实现对应字段改变只渲染部分组件（React 渲染优化）？复杂表单动态输入项 / 状态管理与校验？
...
<!-- niuke-id:NQ-097 source-line:136 -->

**问题：** 框架和 jQuery 的区别？vue 可以用 jsx 吗？如果给多个项的列表通过操作 DOM 插入 body 不能包 div 怎么做？
