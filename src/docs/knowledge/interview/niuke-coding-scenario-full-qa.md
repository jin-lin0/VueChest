...
<!-- niuke-id:NQ-277 source-line:394 -->

**问题：** 实现一个并发控制函数 / 异步调度器 Scheduler（控制最大并发数，结果顺序不变，任务失败不中断，收集所有结果）
...
<!-- niuke-id:NQ-278 source-line:395 -->

**问题：** 带最大并发数限制的 Promise.all / async pool / 竞态请求只消费最后一次（p-limit）
...
<!-- niuke-id:NQ-279 source-line:396 -->

**问题：** 实现 Promise.resolve 及手写 Promise 核心逻辑 / 手写 Promise.all
...
<!-- niuke-id:NQ-280 source-line:397 -->

**问题：** 手写发布订阅模式（含带 once 的 EventEmitter）
...
<!-- niuke-id:NQ-281 source-line:398 -->

**问题：** 串行执行传入的 promises（区别于 Promise.all）
...
<!-- niuke-id:NQ-282 source-line:399 -->

**问题：** 手写 bind（可用 apply / call，不能用展开符）/ 手写 call、apply
...
<!-- niuke-id:NQ-283 source-line:400 -->
...
<!-- niuke-id:NQ-284 source-line:401 -->

**问题：** 手写类型判断 Object.prototype.toString.call
...
<!-- niuke-id:NQ-285 source-line:402 -->

**问题：** LazyMan（链式调用调度）/ hardman 经典题 / 手写类似柯里化求积
...
<!-- niuke-id:NQ-286 source-line:403 -->

**问题：** 手写 repeat(func, times, wait) / 实现定时器 sleep 函数
...
<!-- niuke-id:NQ-287 source-line:404 -->

**问题：** 单例模式 / 实现一个单例 object（双检查锁）
...
<!-- niuke-id:NQ-288 source-line:408 -->

**问题：** 手写深拷贝 / 浅拷贝（循环引用用 WeakMap；ES6 展开运算符是深拷贝还是浅拷贝）
...
<!-- niuke-id:NQ-289 source-line:409 -->

**问题：** 手写数组原地去重 / 数组转树 / 树转扁平化（tree <-> flat）/ 嵌套对象（树）转数组
...
<!-- niuke-id:NQ-290 source-line:410 -->

**问题：** 实现 pick / 实现 classnames 函数 / 实现 once 函数（记忆化第一次执行结果）
...
<!-- niuke-id:NQ-291 source-line:411 -->

**问题：** 实现 myPromiseMap（数组、回调、最大并发，返回结果数组）/ 统计数组元素出现频次并筛选频次 ≥ N
...
<!-- niuke-id:NQ-292 source-line:412 -->

**问题：** 手写：字符串中提取所有数字（含科学计数法、小数、分数、负数）/ 解析 URL 查询字符串为 JSON（含嵌套、类型转换）
...
<!-- niuke-id:NQ-293 source-line:413 -->

**问题：** 随机生成密码（至少一个大写一个小写一个数字一个特殊符号）/ 特殊路径找嵌套对象
...
<!-- niuke-id:NQ-294 source-line:414 -->
...
<!-- niuke-id:NQ-295 source-line:415 -->

**问题：** 反转字符串（多种办法）/ 手写反转链表（迭代 + 递归）/ 反转链表 K 个一组 / 重排链表 / 合并两个有序链表 / 原地合并两个有序数组 / 删除链表倒数第 K 个节点 / 判断链表是否有环（快慢指针）
...
<!-- niuke-id:NQ-297 source-line:417 -->

**问题：** 判断一个字符串是不是累加数 / 判断一个字符串是否是正确的有序括号
...
<!-- niuke-id:NQ-298 source-line:418 -->

**问题：** 给定算术表达式字符串求值（hard）/ 迷宫题
...
<!-- niuke-id:NQ-299 source-line:419 -->

**问题：** 实现倒计时（秒数转 时分秒 + 开始 + 复位）/ 实现深拷贝与浅拷贝（循环引用）

**面试者标准回答：**

> “倒计时用剩余毫秒作为单一真相，每次根据目标结束时间计算，避免 setInterval 漂移累积；start/reset 都要清理旧计时器。深拷贝使用 NQ-288 的 WeakMap 版本，而浅拷贝只复制第一层。”
...
<!-- niuke-id:NQ-300 source-line:420 -->

**问题：** 类继承：实现 车 / 汽车 / 单车 类，含成员属性、方法、默认黑色属性
...
<!-- niuke-id:NQ-301 source-line:421 -->

**问题：** 手写：实现一个简单的 Set 类（add / remove）/ 实现深层比较 isEqual 函数
...
<!-- niuke-id:NQ-302 source-line:422 -->

**问题：** 用 JSON.parse(JSON.stringify()) 做深拷贝有什么局限性（函数 / Symbol / 循环引用 / undefined / Date 等丢失）？

**面试者标准回答：**

> “JSON 往返只适合 JSON 数据，不是通用深拷贝：undefined、函数和 Symbol 会丢失，NaN/Infinity 变成 null，Date 变字符串，Map/Set 变空对象，BigInt 和循环引用会抛错，原型、描述符及对象共享关系也丢失。支持的平台优先用 structuredClone；需要自定义范围时用 NQ-288 的实现。”
...
<!-- niuke-id:NQ-303 source-line:423 -->

**问题：** 最长公共子串（LeetCode 1143 / 718）如何实现？
...
<!-- niuke-id:NQ-304 source-line:424 -->

**问题：** 找出 1-N（如 1-10000）内的所有质数（埃氏筛 / 欧拉筛）？
...
<!-- niuke-id:NQ-305 source-line:425 -->

**问题：** 求二叉树中第 N 大的数（BST 反中序遍历 / 普通二叉树 + 堆）？
...
<!-- niuke-id:NQ-306 source-line:426 -->

**问题：** 字符串压缩：把 aabcccaaaa 压缩成 a2bc3a4（连续相同字符计数，只出现一次不加数字）？
...
<!-- niuke-id:NQ-307 source-line:427 -->

**问题：** 给定两个数组，一个是入口文件的依赖关系、一个是参与打包的所有文件的依赖关系，输出最终打包结果（依赖图遍历 + 去重 / 循环依赖处理）？
...
<!-- niuke-id:NQ-308 source-line:428 -->

**问题：** 找岛屿 / 岛屿数量（DFS 淹没法 与 BFS 两种写法）？
...
<!-- niuke-id:NQ-309 source-line:429 -->

**问题：** 链表排序（对链表做归并排序，ACM 输入输出格式）？其中的 dummy 哨兵节点是必须的吗、为什么选归并而不是快排？
...
<!-- niuke-id:NQ-310 source-line:430 -->

**问题：** 二叉树的锯齿形（Z 字形）层序遍历如何实现（奇数层正序、偶数层逆序，用双端队列或标记层级方向）？
...
<!-- niuke-id:NQ-311 source-line:431 -->

**问题：** 合并区间（LeetCode 56，给定若干区间，合并所有重叠区间并输出）？
...
<!-- niuke-id:NQ-312 source-line:435 -->

**问题：** 事件循环执行顺序输出（多道代码题）/ 给出代码判断 Promise / apply / bind 的输出
...
<!-- niuke-id:NQ-313 source-line:436 -->

**问题：** 提取 URL 中 ? 后和 # 后的参数 / 解析 URL（手撕）
...
<!-- niuke-id:NQ-314 source-line:437 -->

**问题：** 手写 SSE 解析器、按 token 流式渲染 Markdown；流式场景下 Markdown 表格如何判断"完整解析"再一次性渲染？HTML 标签流式缓冲区处理？
...
<!-- niuke-id:NQ-315 source-line:438 -->

**问题：** 推流场景的截流、缓冲区管理、流式数据消费 / AI 流式返回数据的打字机式传输；后端一直吐数据如何保证前端不卡（Web Worker 释放计算线程）
...
<!-- niuke-id:NQ-316 source-line:439 -->

**问题：** 虚拟列表 / 几十万行文本渲染卡顿优化（虚拟滚动 / 分片渲染 / DocumentFragment）；表格合并单元格时懒加载设计；长列表遍历阻塞主线程用 Web Worker
...
<!-- niuke-id:NQ-317 source-line:440 -->

**问题：** 搜索框设计 / 输入关键词推荐 / 搜索结果请求错乱（第二个先回覆盖第一个）怎么解决 / 搜索字符匹配高亮后端如何处理 / 防止用户连续点击造成多次请求
...
<!-- niuke-id:NQ-318 source-line:441 -->

**问题：** 表单校验（含异步校验）/ 动态表单（新增动态输入项）/ 设计一个分页组件（受控还是非受控）/ 复杂表单状态管理与校验
...
<!-- niuke-id:NQ-319 source-line:442 -->

**问题：** 如何实现表单校验（一面）/ 设计一个数据聚合优化方案 / ECharts 数据聚合实现方式
...
<!-- niuke-id:NQ-320 source-line:443 -->

**问题：** 多租户前端改造怎么设计 / 第三方页面 iframe 集成 + 按权限控制展示 / 多系统 API 数据结构（camelCase / snake_case / 中文 key）不统一如何复用 / 20+ 页面经常改字段组件如何设计 / 拖拽式 Dashboard 系统设计
...
<!-- niuke-id:NQ-321 source-line:444 -->

**问题：** 300 辆车实时位置 + 轨迹优化（绕圈 / 锯齿处理、GeoJSON、行政区数据裁剪）/ WebSocket 多连接（车辆位置 / 报警 / 设备状态）复用 / 图表与地图数据级联联动
...
<!-- niuke-id:NQ-322 source-line:445 -->

**问题：** 路由配置化 JSON 结构包含哪些属性 / 对象转字符串 JSON.stringify 何时报错（循环引用）及如何判断 / 微前端自动创建子应用更自动化方式 / 用 AI 现场开发动态表单
...
<!-- niuke-id:NQ-323 source-line:446 -->

**问题：** 前端路由系统（hash / history）实现、路由懒加载、页面过渡动画 / SPA 优化方向 / 弱网降级
...
<!-- niuke-id:NQ-324 source-line:447 -->

**问题：** 实时同步服务端数据显示 / 修复数据更新不同步 / 实时音频通话跟大模型交互时延控制 / 现场用 websocket 完成前端实时更新排序的记分面板（含异常处理 / 心跳机制）/ 直播评论前端设计（成千上万条不卡顿）
...
<!-- niuke-id:NQ-325 source-line:448 -->

**问题：** 高并发场景：1s 内对数据库进行 10w 次写入如何优化 / 后端高并发 1w 接口 down 了怎么处理 / 多用户并发操作同一资源竞态冲突（前端锁 / 服务锁 / 分布式锁）
...
<!-- niuke-id:NQ-326 source-line:449 -->

**问题：** 大文件分片上传、断点续传、进度同步一致性 / 大文件上传弱网怎么办 / 文件下载断点续传（HTTP Range）
...
<!-- niuke-id:NQ-327 source-line:450 -->

**问题：** 从零手写轻量业务状态机（核心模块与容错逻辑）/ 项目状态机六大状态与流转、指数退避重试、熔断
...
<!-- niuke-id:NQ-328 source-line:451 -->

**问题：** 一个死锁（多线程）/ 多线程竞争状态（互斥锁）/ 数组和链表区别 / 哈希表（hashMap）原理
...
<!-- niuke-id:NQ-329 source-line:452 -->

**问题：** 原生 HTML 实现可编辑数据表格的增删改查（新增 / 编辑 / 删除行，斑马线背景，表头点击正序倒序排序，删除弹确认框，鼠标移入显示操作列）？
...
<!-- niuke-id:NQ-330 source-line:453 -->

**问题：** 笔试逻辑题：两个 8 斤桶 + 一个 3 斤桶，将 16 斤水平均倒入 4 个水池（各 4 斤）如何操作？
...
<!-- niuke-id:NQ-331 source-line:454 -->

**问题：** 笔试逻辑题：两两 PK、输的人休息，小明玩 15 局、小刚 21 局、小强休息 5 局，问某一局是谁和谁 PK？
...
<!-- niuke-id:NQ-332 source-line:455 -->

**问题：** 封装一个 v-scroll 自定义滚动指令的思路是什么？触底加载（是否到底、何时触发请求、重复触发如何拦截）的逻辑怎么实现？
...
<!-- niuke-id:NQ-333 source-line:456 -->

**问题：** 让你封装一个弹出框（Modal）组件，说说你的封装思路（挂载方式 / 层级 / 遮罩 / 关闭时机 / API 设计）？不依赖框架用原生怎么实现？
...
<!-- niuke-id:NQ-334 source-line:457 -->

**问题：** 手写代码：父组件下有两个子组件，子组件 A 展示数字、子组件 B 是按钮，通过父组件传递实现点击按钮改变数字（状态提升与父子通信）
...
<!-- niuke-id:NQ-335 source-line:458 -->

**问题：** 拖拽画布 / 低代码编辑器：平台是基于 DOM、SVG 还是 Canvas 实现的、怎么做渲染选型？画布上有上百个组件、缩放到最小时视口内要展示多少组件、性能怎么保证？组件的自动吸附 / 对齐辅助线能力是怎么实现的？
...
<!-- niuke-id:NQ-336 source-line:459 -->

**问题：** 自定义 JSON 编码协议 / 数据结构设计时，为什么用扁平数组而不是嵌套数组（树）？嵌套树结构在什么场景下会更好（可读性、增删改查效率、序列化体积、层级关系表达）？
