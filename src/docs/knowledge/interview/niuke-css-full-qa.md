...
<!-- niuke-id:NQ-098 source-line:142 -->

**问题：** 浏览器页面渲染流程是什么？HTML 解析过程中遇到 script 会发生什么？输入 URL 到页面渲染的完整过程（含 DOM/CSSOM/渲染树/布局/绘制、重排重绘例子）？
...
<!-- niuke-id:NQ-099 source-line:143 -->

**问题：** 什么是重排（reflow）和重绘（repaint）？如何减少？合成层（GPU 合成层）如何提升渲染性能？如何开启合成层？离屏渲染是什么、有什么缺点？
...
<!-- niuke-id:NQ-100 source-line:144 -->

**问题：** 水平垂直居中有哪些方法？为什么用 transform？相对哪个元素？有没有不改动父元素的方法？浮动怎么实现居中？
...
<!-- niuke-id:NQ-101 source-line:145 -->

**问题：** 盒模型是什么、由什么组成？box-sizing 的作用？text-indent 是否计算在盒模型内？position 默认值、各定位值与特点（relative 是否脱离文档流、absolute 与 fixed 区别、fixed 相对什么定位）？
...
<!-- niuke-id:NQ-102 source-line:146 -->

**问题：** flex:1 对应的三个参数（grow / shrink / basis）？flex 如何垂直水平居中？Flex 布局原理、常用属性？不知道子元素数量时如何让子元素等间距 / 等宽排列？
...
<!-- niuke-id:NQ-103 source-line:147 -->

**问题：** 三栏布局有哪些实现方式（双飞翼 / 两侧固定中间自适应）？
...
<!-- niuke-id:NQ-104 source-line:148 -->

**问题：** BFC 是什么、特点？清除浮动 / 高度塌陷、clear 的使用？margin 合并现象（垂直合并）？
...
<!-- niuke-id:NQ-105 source-line:149 -->

**问题：** 不用 border-radius 如何实现一个圆？如何用 CSS 做成一个三角形、并让它保持旋转？
...
<!-- niuke-id:NQ-106 source-line:150 -->

**问题：** 行内元素与块级元素分别有哪些？display:none 与 visibility:hidden、opacity:0 的核心区别？让页面元素隐藏的几种方式？
...
<!-- niuke-id:NQ-107 source-line:151 -->

**问题：** HTML 的 head / body？meta / link / style？link 与 @import 的区别？link 可引入哪些资源、type 做什么、能否跨域？script 跨域吗、async / defer 区别？JS 放在 HTML 前面且无异步属性会造成什么后果？JS 报错会对页面渲染产生什么影响？
...
<!-- niuke-id:NQ-108 source-line:152 -->

**问题：** CSS 选择器有哪些、优先级？!important 优先级？BEM 命名规范的应用场景与优缺点？
...
<!-- niuke-id:NQ-109 source-line:153 -->

**问题：** CSS 长度单位（px / em / rem / vw / vh / rpx / vmax / vmin / 百分比）？10vh 含义？rpx 用过吗？
...
<!-- niuke-id:NQ-110 source-line:154 -->

**问题：** 移动端适配方式（媒体查询、rem、vw/vh、px/em/rem 区别、iOS 安卓差异）？移动端安全区域（刘海 / 底部小黑条）适配（env(safe-area-inset-\*) / SafeAreaView）？
...
<!-- niuke-id:NQ-111 source-line:155 -->

**问题：** 移动端 / 前端画圆的方式？前端拖动轴 / 滑块动态显隐怎么设置？CSS 里能写脚本吗？
...
<!-- niuke-id:NQ-112 source-line:156 -->

**问题：** 实现一个 button 渐变效果（深红到浅红）的多种方式？鼠标的拖尾效果怎么实现？
...
<!-- niuke-id:NQ-113 source-line:157 -->
...
<!-- niuke-id:NQ-114 source-line:158 -->

**问题：** 定义一个动画有哪几种方式（animation / transform / CSS3 动画）？轮播图过渡效果？用 CSS 实现点击方块动态设置背景色、animation 简单动画？
...
<!-- niuke-id:NQ-115 source-line:159 -->

**问题：** 响应式布局的方式（媒体查询、rem 等）？设计封装响应式布局的组件？i18n 原理及性能优化？
...
<!-- niuke-id:NQ-116 source-line:160 -->

**问题：** 图片格式 jpg / svg / png / webp 优缺点与适用场景？为什么 WebP 而不用 PNG/JPG？Base64 的作用？
...
<!-- niuke-id:NQ-117 source-line:161 -->

**问题：** canvas 与 svg 的区别及各自用途？canvas 绘制点 / 线、如何给 canvas 添加点击事件、如何绘制 5 万条数据、地图绘制大量点为什么影响性能？svg 节点渲染卡顿如何优化？
...
<!-- niuke-id:NQ-118 source-line:162 -->

**问题：** 暗黑 / 亮色主题切换怎么实现？PC 端暗黑模式怎么适配？
...
<!-- niuke-id:NQ-119 source-line:163 -->

**问题：** 如何做移动端 / H5 适配、iOS 安卓端适配差异？如何做 SEO 规范？
...
<!-- niuke-id:NQ-120 source-line:164 -->

**问题：** DOM 事件流动机制（捕获和冒泡）？事件冒泡和事件捕获？父元素触发的事件怎么防止传递给子元素？事件流、事件委托、addEventListener 的 capture / 冒泡、第三个参数（useCapture）？处于事件阶段？
...
<!-- niuke-id:NQ-121 source-line:165 -->

**问题：** 大模型输出快慢对 RAF 的优劣势、如何平衡不同输出频率下 RAF 的表现？requestAnimationFrame 是什么、作用？
...
<!-- niuke-id:NQ-122 source-line:166 -->

**问题：** 页面抖动 bug 怎么处理？平时怎么定位和修改样式问题？
...
<!-- niuke-id:NQ-123 source-line:167 -->

**问题：** Three.js / WebGL 项目怎么做离屏渲染？还做了哪些渲染优化？切换场景时如何防止内存泄漏（材质 / 几何体 / 纹理是否需要手动释放）？
...
<!-- niuke-id:NQ-124 source-line:168 -->

**问题：** 常用的 CSS 布局方案有哪些？Flex 和 Grid 的区别是什么、分别适合什么场景？
...
<!-- niuke-id:NQ-125 source-line:169 -->

**问题：** CSS 场景题：顶部导航条 + 中间内容区 + 底部 footer，内容区较短时 footer 紧贴内容区、内容区较长时 footer 固定在页面底部且内容区占满剩余高度并在自身内部滚动，怎么实现（sticky footer / flex 布局）？
