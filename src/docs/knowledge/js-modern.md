...
results.forEach(r => r.status === 'fulfilled'
  ? console.log('OK', r.value) : console.warn('失败', r.reason));
...
```js
const logs = [{ t: 1, ok: true }, { t: 2, ok: false }, { t: 3, ok: false }];
...
// 默认参数（注意可引用前面的参数）
function greet(name = '游客', prefix = `Hi ${name}`) { return `${prefix}, ${name}`; }

// 箭头函数无自己的 this，继承外层 —— 适合回调
const obj = { n: 1, inc() { setTimeout(() => console.log(this.n++), 0); } };
...
// 带错误兜底
let lib;
try { lib = await import('./optional.js'); }
catch { lib = fallbackImpl; }
```
