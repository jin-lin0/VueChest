...
> 面向 2024–2026 前端 / 大厂社招校招的「**手写场景题 / 代码实现题**」：不是纯算法，而是考察**工程能力、语言功底与框架原理**的现场编码。每题均含**原题描述、标准答案（完整 JS 代码）与关键说明**，可直接用于自测与复盘。
...
function throttle(fn, wait) {
  ...
  return function (...args) {
    ...
    if (remaining <= 0) { // 超过间隔，立即执行
      if (timer) { clearTimeout(timer); timer = null }
      last = now
      fn.apply(this, args)
    } else if (!timer) { // 间隔内，末次补一次
      timer = setTimeout(() => {
  ...
  const clone = Array.isArray(obj)
    ...
    : obj instanceof Map
      ...
      : obj instanceof Set
        ...
        : {}
  cache.set(obj, clone)
  if (obj instanceof Map) { for (const [k, v] of obj) clone.set(deepClone(k, cache), deepClone(v, cache)) }
  else if (obj instanceof Set) { for (const v of obj) clone.add(deepClone(v, cache)) }
  else { for (const key in obj) if (obj.hasOwnProperty(key)) clone[key] = deepClone(obj[key], cache) }
  return clone
}
```
...
function jsonp(url, { timeout = 5000, params = {} } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const clean = () => { delete window[cbName]; script.remove() }
    const timer = setTimeout(() => { clean(); reject(new Error('timeout')) }, timeout)
    window[cbName] = (data) => { clearTimeout(timer); clean(); resolve(data) }
    const qs = Object.entries({ ...params, callback: cbName })
      .map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join('&')
    script.src = url + (url.includes('?') ? '&' : '?') + qs
    script.onerror = () => { clearTimeout(timer); clean(); reject(new Error('script error')) }
  ...
  return new Promise((resolve) => {
    ...
    arr.forEach((p, i) => {
      Promise.resolve(p).then(
        (v) => { res[i] = { status: 'fulfilled', value: v } },
        (r) => { res[i] = { status: 'rejected', reason: r } }
      ).finally(() => { if (++count === arr.length) resolve(res) })
    })
  })
}
```
...
async function trafficLight() {
  while (true) {
    console.log('🟢 绿灯'); await sleep(3000)
    console.log('🟡 黄灯'); await sleep(1000)
    console.log('🔴 红灯'); await sleep(2000)
  }
}
```
时间复杂度 O(n)，空间复杂度 O(1)。
...
function reactive(obj) {
  ...
  const trigger = (key) => { depsMap.get(key)?.forEach((fn) => fn()) }
...
class EventEmitter {
  ...
  once(event, cb) {
    const wrap = (...args) => { cb(...args); this.off(event, wrap) }
...
function useThrottledCallback(fn, delay = 300) {
  ...
  return useCallback((...args) => {
    ...
    if (remaining <= 0) { last.current = now; fn(...args) }
    else if (!timer.current) {
      timer.current = setTimeout(() => { last.current = Date.now(); timer.current = null; fn(...args) }, remaining)
    }
  }, [fn, delay])
}
```
...
function useFetch(url) {
  ...
  useEffect(() => {
    ...
    return () => { ignore = true; controller.abort() } // 卸载/url 变化取消
  }, [url])
