...
defineOptions({ name: 'TomlTool' })

type Dir = 'toml2json' | 'json2toml'
const dir = ref<Dir>('toml2json')
const input = ref('')
const output = ref('')
const error = ref('')
...
const run = debounce(() => {
  ...
  try {
    if (dir.value === 'toml2json') {
      const obj = parseToml(input.value)
      output.value = JSON.stringify(obj, null, 2)
    } else {
      const obj = JSON.parse(input.value)
      if (obj === null || typeof obj !== 'object' || Array.isArray(obj)) {
        throw new Error('JSON→TOML 需要输入一个 JSON 对象（键值对）')
      }
      output.value = stringifyToml(obj)
    }
  } catch (e) {
    output.value = ''
...
<template>
  <div class="toml-app">
    <div class="toolbar">
      <div class="seg">
        <button :class="{ active: dir === 'toml2json' }" @click="dir = 'toml2json'">
          TOML → JSON
        </button>
        <button :class="{ active: dir === 'json2toml' }" @click="dir = 'json2toml'">
          JSON → TOML
        </button>
      </div>
    ...
    <div class="grid">
      <section class="card">
        <div class="card-head">
          <span class="card-title">{{ dir === 'toml2json' ? 'TOML' : 'JSON' }}</span>
          <span class="hint">输入即转换（Ctrl/Cmd+S 亦触发）</span>
        </div>
        <div class="editor-wrap">
          <CodeEditor
            v-model="input"
            :language="dir === 'toml2json' ? 'plaintext' : 'json'"
      ...
      <section class="card output">
        <div class="card-head">
          <span class="card-title">{{ dir === 'toml2json' ? 'JSON' : 'TOML' }}（只读）</span>
        </div>
        <div class="editor-wrap">
          <CodeEditor
            v-model="output"
            :language="dir === 'toml2json' ? 'json' : 'plaintext'"
            readonly
            placeholder="结果将显示在此"
          />
        </div>
